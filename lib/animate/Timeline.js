...
class Timeline extends Array {
  ...
  addTween(properties, startFrame, duration, ease) {
    ...
    const tween = new Tween.Tween(this.target, startProps, properties, startFrame, duration, ease);
    ...
    if (this.length && this[this.length - 1].startFrame >= startFrame) {
      for (let i = this.length - 1; i >= 0; --i) {
        ...
        if (prev.startFrame === startFrame) {
          ...
          const tween = new Tween.Tween(this.target, startProps, null, startFrame, duration);
